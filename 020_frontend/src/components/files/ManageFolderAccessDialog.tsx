...
interface ManageFolderAccessDialogProps {
    open: boolean;
...
export function ManageFolderAccessDialog({ open, onOpenChange, folder }: ManageFolderAccessDialogProps) {
    ...
    useEffect(() => {
        if (open && folder) {
            // Check if folder has access rules
            const hasRules = folder.accessRules && folder.accessRules.length > 0;

            // Check for admin-only mode (special marker rule)
            const isAdminOnly = folder.accessRules?.some((r: any) =>
            ...
            folder.accessRules?.forEach((rule: any) => {
    ...
    const mutation = useMutation({
        mutationFn: (accessRules: any[]) => fileService.updateFolderAccess(folder!.id, accessRules),
        onSuccess: () => {
    ...
    return (
        <Dialog open={open} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>Ordner-Berechtigungen: {folder?.name}</DialogTitle>
                </DialogHeader>
