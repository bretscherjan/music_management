  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        {accentMap.map((level, i) => {
          ...
          return (
            <div
              ...
              style={{
                background: isActive
                  ? (level === 2 ? activeCatColor : level === 1 ? `${activeCatColor}cc` : '#BDD18C')
                  : '#e5e7eb',
      ...
      <div className="flex items-center gap-3">
        ...
        <input type="range" min={20} max={300} value={bpm} onChange={e => setBpm(Number(e.target.value))} className="flex-1 accent-[#BDD18C] h-2" />
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {[60, 72, 80, 90, 96, 100, 108, 120, 132, 144, 160].map(b => (
          <button key={b} onClick={() => setBpm(b)} className={cn('px-2.5 py-1 rounded-md text-xs font-medium border transition-all', bpm === b ? 'bg-[#BDD18C] text-[#405116] border-[#405116]/20' : 'bg-white text-gray-500 border-gray-200 hover:border-[#BDD18C]')}>{b}</button>
        ))}
      </div>
      ...
      <div className="flex gap-3 justify-center">
        ...
        <Button onClick={handleTap} variant="outline" size="lg" className="gap-2 px-6 border-[#BDD18C] text-[#405116] hover:bg-[#BDD18C]/10 active:scale-95 transition-all select-none">
